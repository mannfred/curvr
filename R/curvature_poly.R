...
#'
#' @param param whether to parameterize the curve by arc length
#' (see\code{pracma::arclength}). Setting \code{param=TRUE} greatly
#' increases computing time.
#'
#' @return a numeric representing total curvature. Units are radians.
#'
#' @examples
#'
#' library(Momocs)
...
curvature_poly <- function(Momocs_poly, x_range, subdiv = 1000, param = FALSE) {
  ...
  i.e. a list object generated by Momocs::npoly or Momocs::opoly.
  See: '?Momocs::npoly'.")
  }
  ...
  if (attr(dfun(x_range[1]), "gradient") == attr(
    dfun(x_range[2]),
    "gradient"
  )) {
  ...
  if (param == FALSE) {
    ...
    arc <- pracma::arclength(
      param_poly, x_range[1],
      x_range[2]
    )$length
